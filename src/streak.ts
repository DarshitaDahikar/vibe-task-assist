...
/** Call whenever a task is marked complete. Increments the streak once per calendar day. */
export function recordCompletion(): number {
  const data = readStreak();
  const today = todayISO();

  if (data.lastDate === today) {
    // Already recorded a completion today — no change.
    return data.count;
  }

  const continuing = data.lastDate === isoYesterday();
  const next: StreakData = {
    count: continuing ? data.count + 1 : 1,
    lastDate: today,
  };
  writeStreak(next);
  return next.count;
...
/** "Good morning", "Good afternoon", or "Good evening" based on the local hour. */
